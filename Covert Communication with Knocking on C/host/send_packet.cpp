...
void forgepacket(char src_host[],char dst_host[],int dst_port,char* payload, int ipid){
  //Built TCP packet
  ...
  //TCP Header
  tcph->source = htons (1+(int)(10000.0*rand()/(RAND_MAX+1.0)));
...
void forgedatagram(char src_host[],char dst_host[],int dst_port,char* payload, int ipid){
  //Built UDP packet
  char datagram[4096] , source_ip[32] , *data , *pseudogram;
  int s = socket (PF_INET, SOCK_RAW, IPPROTO_TCP);
  struct iphdr *iph = (struct iphdr *) datagram;
